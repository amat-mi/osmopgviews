...
class RawView(object):
    ...
    def build_sql(self):
        ...
        if self.where:
            ...
            if mft != '':
                mft = mft + ', '
            sql = 'select {fk}::character varying(16), {meta_fields}{geom_field} as geom, {fields} from ({sql}) as q where ({where})'.format(fk=self.tags_fk,  fields=','.join(field_list),sql=sql, where=self.where, meta_fields=mft, geom_field=self.geom_field)
        return sql
